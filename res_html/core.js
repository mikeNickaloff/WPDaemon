...
w3.filterHTML = function (id, sel, filter) {
    ...
    for (i = 0; i < a.length; i++) {
        ...
        for (ii = 0; ii < b.length; ii++) {
            hit = 0
            if (b[ii].innerHTML.toUpperCase().indexOf(filter.toUpperCase(
                                                          )) > -1) {
                hit = 1
            }
            c = b[ii].getElementsByTagName("*")
            for (iii = 0; iii < c.length; iii++) {
                if (c[iii].innerHTML.toUpperCase().indexOf(filter.toUpperCase(
                                                               )) > -1) {
                    hit = 1
                }
            }
        ...
        for (j = 0; j < 2; j++) {
            ...
            while (y == 1) {
                ...
                for (ii = 0; ii < (b.length - 1); ii++) {
                    bytt = 0
                    if (sortvalue) {
                        v1 = b[ii].querySelector(
                                    sortvalue).innerHTML.toLowerCase()
                        v2 = b[ii + 1].querySelector(
                                    sortvalue).innerHTML.toLowerCase()
                    } else {
    ...
    for (j = (l - 1); j >= 0; j -= 1) {
        ...
        if (repeatObj && typeof repeatObj == "object"
            ...
            for (x in repeatObj) {
                i += 1
                rowClone = arr[j]
                rowClone = w3_replace_curly(rowClone, "element", repeatX,
                                            repeatObj[x])
                a = rowClone.attributes
                for (ii = 0; ii < a.length; ii += 1) {
                    a[ii].value = w3_replace_curly(a[ii], "attribute", repeatX,
                                                   repeatObj[x]).value
                }
                (i === repeatObj.length) ? arr[j].parentNode.replaceChild(
                                               rowClone,
                                               arr[j]) : arr[j].parentNode.insertBefore(
                                               rowClone, arr[j])
            }
        } else {
